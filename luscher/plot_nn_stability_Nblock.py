...
def main():
    ...
    parser.add_argument('--tmin',   nargs='+', default=range(2,9),
    ...
    nn_dict = { 'N_t':N_t, 't_norm':tnorm, 'nn_iso':nn_iso, 'BLOCK':block}
    ...
    summary_plot.summary_ENN(blk_results, mN, blk_lbls, color, 
                             lbl0=r'$N_{\rm block}$=', spin=nn_iso, fig=f"{nn_iso}_Nb_summary")
...
def plot_tmin(axnn, axnnR, axQ, state, models, arg, nnFile, nnDict, nnModel, optModel, fitKeys, nnData, r_blk, l_blk, ratio=True):
    ...
    axnn.legend(flip(handles, len(arg.Nb)), flip(labels, len(arg.Nb)), loc=1, ncol=len(arg.Nb), fontsize=16, columnspacing=0,handletextpad=0.1)
...
def plot_one_tmin(t, axnn, axnnR, axQ, state, models, arg, nnFile, nnDict, nnModel, optModel, fitKeys, nnData, r_blk, l_blk):
    ...
    for model in models:
        for Nb in arg.Nb:
            ...
            if t == arg.tmin[0]:
                if Nb == arg.Nb[0]:
                    lbl = r'$N_{\rm inel} = %d, N_b = %d$' %(n_inel, Nb)
                else:
                    lbl = r'$%d, %d$' %(n_inel, Nb)
            else:
                lbl = ''
            ...
            if os.path.exists(fit_file):
                ...
                c_plot.append(color[Nb])
                t_plot.append(t+shift[Nb])
                mfc='None'
                if t == opt_tmin and fit_model == optModel and Nb==arg.Nb[0]:
                    mfc='k'#clr
                mfc_plot.append(mfc)
                axnnR.errorbar(t+shift[Nb],
                                e[-1].mean, yerr=e[-1].sdev,
                                marker=mrkr, color=clr, mfc=mfc,
                                linestyle='None',label=lbl)
                axnn.errorbar(t+shift[Nb],
