...
def main():
    ...
    parser.add_argument('--gevp',   nargs='+', type=str, default=['5-10'],
    ...
    Nn_plot   = int(args.optimal.split('_N_n')[1].split('_')[0])
    ...
    for q in states:
        ...
        Nn_results['_'.join([str(k) for k in q])] = {'DE':[],'E1':[],'E2':[]}
        print(q)
...
def plot_tmin(axnn, axnnR, axQ, state, models, arg, nnFile, nnDict, nnModel, optModel, fitKeys, nnData, r_Nn, l_Nn, ratio=True):

    q_str = '\_'.join([str(k) for k in state])

    for t in arg.tmin:
        plot_one_tmin(t, axnn, axnnR, axQ, state, models, arg, nnFile, nnDict, nnModel, optModel, fitKeys, nnData, r_Nn, l_Nn)
    ...
    nnr_lim = {
        ...
        ('0', 'T1g', 0):(-0.0017,0.000), ('0', 'T1g', 1):(-0.0051,0.0005),
