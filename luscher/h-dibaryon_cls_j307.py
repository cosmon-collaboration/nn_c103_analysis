...
def params():
   ...
   """
    ...
    p["fpath"] = {"nucleon": f"./data/nucleon_{ens}.hdf5", 
                  "nn": f"./data/triplet_{ens}.hdf5"}
    p['rw']    = {'file':f'data/rw_{ens}.hdf5',
                  'hpath':'rw'}

    p["save"]   = True
    p["result"] = 'result_'+ens
    ...
    p['get_Zj'] = True
    p['Zjn_values'] = f"result_{ens}/{p['fpath']['nn'].split('/')[-1].split('_')[0]}"
    ...
    p["autotime"]   = 6 # time used to estimate mean gs energy prior
