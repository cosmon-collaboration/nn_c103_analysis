...
def main():
    ...
    parser.add_argument('--tmin',   nargs='+', type=int, default=[2,3,4],
    ...
    nn_file += '_NN_{nn_model}_e{nn_el}_t_{t0}-{tf_NN}_ratio_'+str(args.ratio)+'.pickle'
...
def plot_one_tmin(t, axnn, axnnR, axQ, state, models, arg, nnFile, nnDict, nnModel, optModel, fitKeys, nnData):
    ...
    for model in models:
        for t0 in arg.tmin:
            ...
            if t == arg.tf_NN[0]:
                if t0 == arg.tmin[0]:
                    lbl = r'$N_{\rm inel} = %d, t^{NN} = %d-%d$' %(n_inel, t0, t)
                else:
                    lbl = r'$%d, %d-%d$' %(n_inel, t0, t)
            else:
                lbl = ''
