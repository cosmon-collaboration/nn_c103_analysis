...
def main():
    ...
    parser.add_argument('--n_NN',   nargs='+', type=int, default=[2,3,4,6],
                        help=       'number of exponentials in NN to sweep over [%(default)s]')
    ...
    color = {'N_n3_NN_conspire_e0':'b',
             'N_n2_NN_agnostic_n2_e0':'r',
             'N_n2_NN_agnostic_n3_e0':'g',
             'N_n2_NN_agnostic_n4_e0':'orange',
             'N_n3_NN_agnostic_n2_e0':'r',
             'N_n3_NN_agnostic_n3_e0':'g',
             'N_n3_NN_agnostic_n4_e0':'orange',
             'N_n3_NN_agnostic_n6_e0':'magenta',
    }
    ...
    if 'conspire' in args.optimal:
        ...
        sys.exit('you supplied an "agnostic" model, but we require "conspire" for the optimal')
    ...
    # plot t0_N
    opt_spin = args.optimal.split('NN_')[1].split('_')[0]
    summary_plot.summary_ENN(blk_results, mN, blk_lbls, color, 
                             lbl0=r'$N_{\rm block}$=', spin=opt_spin,
                             fig=f"{nn_iso}_agnostic_conspire_summary")
...
def plot_tmin(axnn, axnnR, axQ, state, models, arg, nnFile, nnDict, nnModel, optModel, fitKeys, nnData, r_blk, l_blk, ratio=True):
    ...
    for t in arg.tmin:
        n_models = plot_one_tmin(t, axnn, axnnR, axQ, state, models, arg, nnFile, nnDict, nnModel, optModel, fitKeys, nnData, r_blk, l_blk)
    ...
    axQ.set_ylabel(r'$w_{\rm model}$', fontsize=20)
...
def plot_one_tmin(t, axnn, axnnR, axQ, state, models, arg, nnFile, nnDict, nnModel, optModel, fitKeys, nnData, r_blk, l_blk):
    marker = {
        'N_n3_NN_conspire_e0':'*',
        'N_n2_NN_agnostic_n2_e0':'s',
        'N_n2_NN_agnostic_n3_e0':'d',
        'N_n2_NN_agnostic_n4_e0':'o',
        'N_n3_NN_agnostic_n2_e0':'^',
        'N_n3_NN_agnostic_n3_e0':'<',
        'N_n3_NN_agnostic_n4_e0':'+',
        'N_n3_NN_agnostic_n6_e0':'s',
    }
    shift = {'N_n3_NN_conspire_e0':0,
             'N_n2_NN_agnostic_n2_e0':-0.2,
             'N_n2_NN_agnostic_n3_e0':-0.15,
             'N_n2_NN_agnostic_n4_e0':-0.1,
             'N_n3_NN_agnostic_n2_e0':0.1,
             'N_n3_NN_agnostic_n3_e0':0.15,
             'N_n3_NN_agnostic_n4_e0':0.2,
             'N_n3_NN_agnostic_n6_e0':0.25,
    }
    color = {'N_n3_NN_conspire_e0':'b',
             'N_n2_NN_agnostic_n2_e0':'r',
             'N_n2_NN_agnostic_n3_e0':'g',
    ...
    for model in models:
        ...
        sys.stdout.write('  t=%d, %s\r' %(t, model))
        sys.stdout.flush()
        ...
        nnDict.update({'N_inel':n_inel, 'nn_el':nn_el, 't0':t, 
                        'nn_model':models[model]['nn_model']})

        if t == arg.tmin[0]:
            if 'conspire' in model:
                lbl = r'C: $N_{\rm inel} = %d$' %(n_inel)
            else:
                n_N  = int(model.split('N_n')[1].split('_')[0])
                n_NN = int(model.split('agnostic_n')[1].split('_')[0])
                if n_N == arg.n_N[0] and n_NN == arg.n_NN[0]:
                    lbl = r'A: $N_n=%d, N_{nn}=%d$' %(n_N,n_NN)
                else:
                    lbl = r'A: $%d, %d$' %(n_inel, nn)
        else:
            lbl = ''
        ...
        if os.path.exists(fit_file):
            ...
            try:
                ...
                print(fit_file)
                sys.exit()
            p.append(np.exp(data[((state,), 'logGBF')]))
            k_n = fitKeys[state][0][2]
            k_tmp  = fitKeys[state]
